...
Uma conexão opcional com o TMDB é uma conta no TMDB, não uma conta do Edendale.

## 4. Informações tratadas pelo Edendale

### 4.1 Mídias locais e informações da biblioteca

Quando você escolhe um arquivo, uma pasta ou um compartilhamento de rede, o
Edendale pode tratar:

- nomes de arquivos e pastas;
- caminhos relativos, identificadores de arquivo da plataforma ou marcadores com
  escopo de segurança;
- tipo, tamanho e data de modificação do arquivo;
- o título, o ano de lançamento, o nome da série, o número da temporada e o
  número do episódio deduzidos do nome do arquivo;
- o endereço de um servidor e compartilhamento de rede que você selecionar; e
...
reproduzir a partir de um compartilhamento de rede, seu dispositivo se comunica
...
- um instantâneo de exibição limitado, como um título ou uma referência de
  pôster, usado em widgets da tela de início e nas fileiras de "continuar
  assistindo".
...
Se você conectar uma conta opcional do TMDB, o Edendale recebe um token de
...
armazenamento criptografado apoiado no Keystore do Android no Android e a DPAPI
...
Se você conectar um compartilhamento de rede protegido por senha, o Edendale
guarda o endereço do compartilhamento e as credenciais nesse mesmo
armazenamento protegido. Essas credenciais servem apenas para conectar você ao
compartilhamento escolhido e não são enviadas à BaBaSaMa nem a terceiros.
...
O índice da biblioteca local, os caminhos de arquivo e os marcadores com escopo
...
compartilhamento de rede e do serviço de legendas podem usar o Chaveiro do
...
aplicativo elegíveis no backup da plataforma ou na transferência. Os
armazenamentos protegidos da sessão do TMDB, das credenciais de rede e da chave
de legendas ficam excluídos tanto do backup na nuvem quanto da transferência de
dispositivo pelas regras de backup do Edendale, porque as chaves que os protegem
nunca saem do dispositivo. A proteção e a retenção dos backups dependem da sua
versão do Android, do dispositivo, da conta e do provedor de backup.

### 5.4 Windows

O Windows guarda o índice da biblioteca local no armazenamento local do
aplicativo. O progresso de reprodução, os favoritos, os itens da lista para
assistir e as avaliações também ficam localmente e, quando o OneDrive está
configurado no dispositivo, o Edendale coloca uma réplica desses registros
pessoais na sua própria pasta do OneDrive `Apps/Edendale`, para que um segundo
PC conectado à mesma conta fique em sincronia. Sem o OneDrive, o aplicativo
...
da sua conta Microsoft e suas configurações de privacidade.
...
conforme seus próprios termos e práticas de privacidade, fora do controle da
BaBaSaMa. Você pode evitar totalmente qualquer contato com a Wyzie não iniciando
...
| Conectar a uma conta opcional do TMDB ou a um compartilhamento de rede | Token de conta ou credenciais do compartilhamento | Sua solicitação ou consentimento |
...
desconectando a conta correspondente, desativando o recurso ou alterando as
permissões da plataforma.

## 11. Compartilhamento e prestadores de serviço
...
- ao servidor de rede que você escolhe ao conectar um compartilhamento;
...
- **Android:** uma cópia de backup da plataforma ou de transferência de
  ...
  fabricante do dispositivo ou do seu provedor de backup.
- **Windows:** uma réplica na sua pasta do OneDrive `Apps/Edendale` permanece até
  ...
  recuperação.
- **As legendas baixadas** ficam no seu dispositivo até você removê-las. A Wyzie
...
O Edendale usa conexões de rede criptografadas para os serviços on-line
compatíveis, recusa tráfego sem criptografia e usa o armazenamento protegido da
plataforma para credenciais. O Serviço mantém deliberadamente os dados de vídeo
e os registros pessoais fora de qualquer armazenamento operado pelo
...
compartilhamentos de rede e seus backups.
